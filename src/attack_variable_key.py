...
pt, pl, at, al = load_dataset("variable_key_traces.h5")
...
print(f"\nVariable-Key Attack Accuracy: {acc*100:.2f}%")

avg_rank = key_rank_analysis(model, at, al)
print(f"Average Key Rank: {avg_rank:.2f}")

fixed_model = load_model("model_fixed_key.h5")
_, fixed_acc = fixed_model.evaluate(at, al_cat, verbose=0)
print(f"Fixed-Key Model on Variable-Key Data: {fixed_acc*100:.2f}%")
...
plt.title('Variable Key - Accuracy')
plt.xlabel('Epoch'); plt.ylabel('Accuracy'); plt.legend()
plt.subplot(1,2,2)
plt.plot(history.history['loss'], label='Train')
plt.plot(history.history['val_loss'], label='Val')
plt.title('Variable Key - Loss')
plt.xlabel('Epoch'); plt.ylabel('Loss'); plt.legend()
plt.tight_layout()
plt.savefig("variable_key_training.png", dpi=150)
